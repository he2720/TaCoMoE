...
class Trainer:
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        signature_columns.append('task_id')
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        print('self.args.save_only_model',self.args.save_only_model)
        ...
        print('self.args.should_save',self.args.should_save)
        if self.args.should_save:
            # Update the `TrainerControl` state to where we are currently
            self.state.stateful_callbacks["TrainerControl"] = self.control.state()
            self.state.save_to_json(os.path.join(output_dir, TRAINER_STATE_NAME))
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss,con_loss = self.compute_loss(model, inputs)
        ...
        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            ...
            con_loss = con_loss / self.args.gradient_accumulation_steps 
        ...
        if self.use_apex:
            ...
            self.accelerator.backward(0.02*con_loss,  **kwargs)
            #con_loss = self.deepspeed.backward(0.02*con_loss, retain_graph=True)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        return (loss, outputs, con_loss) if return_outputs else loss,con_loss
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        print('3483',supported_classes)
        print(isinstance(self.model, supported_classes))
    ...
    def predict(
        ...
        start_time = time.time()
        print('3727',self.args.use_legacy_prediction_loop)
        eval_loop = self.prediction_loop if self.args.use_legacy_prediction_loop else self.evaluation_loop
        output = eval_loop(
